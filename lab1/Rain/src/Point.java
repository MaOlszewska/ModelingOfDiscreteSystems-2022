...
public class Point {
	...
	public void calculateNewState() {
		...
		else if(currentState == 0 && neighbors.size() >= 1 && neighbors.get(0).getState() > 0){ nextState = 6; }
	}
	...
	private int countNeighbour() {
		...
		for (Point neighbor : neighbors) { if (neighbor.currentState > 0) aliveNeighbours += 1; }
		return aliveNeighbours;
	}

	public void drop(){
		Random randomNumber = new Random();
		if(randomNumber.nextInt(100) <= 3){
			currentState = 6;
		}
	}
}
